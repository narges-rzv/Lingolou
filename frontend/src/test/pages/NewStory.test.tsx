...
describe('NewStory', () => {
  ...
  it('auto-generates prompt', async () => {
    ...
    await waitFor(() => {
      ...
      expect((promptArea as HTMLTextAreaElement).value).toContain('story')
    })
  })
  ...
  it('reset button restores auto-generated prompt', async () => {
    ...
    const promptArea = screen.getByLabelText(/Generated Prompt/)
    const original = (promptArea as HTMLTextAreaElement).value
    await user.type(promptArea, ' manual edit')

    await user.click(screen.getByText('Reset'))
    expect((promptArea as HTMLTextAreaElement).value).toBe(original)
  })
