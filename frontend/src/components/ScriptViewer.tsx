...
function EditableField({ value, onChange, className, multiline, dir }: EditableFieldProps) {
  if (multiline) {
    return (
      <textarea
...
function newEntry(type: string): ScriptEntry {
  switch (type) {
...
function InsertBar({ onInsert }: InsertBarProps) {
...
interface ScriptEntryComponentProps {
  entry: ScriptEntry;
  index: number;
  editing: boolean;
...
export default function ScriptViewer({ storyId, chapterNumber }: ScriptViewerProps) {
    ...
    apiFetch(`/stories/${storyId}/chapters/${chapterNumber}/script?enhanced=true`)
      .then((data: unknown) => {
        ...
        const arr = Array.isArray(data)
          ? data
          : ((data as Record<string, unknown>).entries || (data as Record<string, unknown>).script || (data as Record<string, unknown>).lines || []) as ScriptEntry[];
  ...
  const handleUpdate = (index: number, updatedEntry: ScriptEntry) => {
    setEntries((prev) => {
      const copy = [...prev];
  ...
  const handleInsert = (afterIndex: number, type: string) => {
    setEntries((prev) => {
      const copy = [...prev];
  ...
  const handleAddAtEnd = (type: string) => {
    setEntries((prev) => [...prev, newEntry(type)]);
    setDirty(true);
  };

  const validateScriptJson = (data: unknown): string | null => {
    ...
    if (typeof data === 'object' && data !== null) {
      ...
      const arr = obj.entries || obj.script || obj.lines;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const scriptObj = script as Record<string, unknown> | null;
      const toSave = Array.isArray(script)
        ? entries
        : { ...scriptObj, [scriptObj?.entries ? 'entries' : scriptObj?.script ? 'script' : scriptObj?.lines ? 'lines' : 'entries']: entries };
