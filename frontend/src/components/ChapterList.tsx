...
export default function ChapterList({ chapters, storyId, autoExpand, onRefresh }: ChapterListProps) {
  const [expanded, setExpanded] = useState<number | null>(null);
  const [chapterTasks, setChapterTasks] = useState<Record<number, string | null>>({});
  ...
  const generateChapterAudio = async (chapterNumber: number) => {
    try {
  ...
  const handleChapterTaskComplete = (chapterNumber: number) => {
    setChapterTasks((prev) => ({ ...prev, [chapterNumber]: null }));
    onRefresh?.();
  };

  const handleChapterTaskError = (chapterNumber: number, msg: string) => {
    setChapterTasks((prev) => ({ ...prev, [chapterNumber]: null }));
    alert(msg);
    onRefresh?.();
  };
