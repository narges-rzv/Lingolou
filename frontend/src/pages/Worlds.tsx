...
import type { WorldListItem, VoiceListItem } from '../types';
...
export default function Worlds() {
  ...
  const [characters, setCharacters] = useState<Array<{ name: string; description: string }>>([{ name: 'NARRATOR', description: 'Tells the story' }]);
  const [voiceConfig, setVoiceConfig] = useState<Record<string, Record<string, unknown>>>({});
  ...
  const handleCreate = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setCreating(true);
    try {
      const charsObj: Record<string, string> = {};
            ...
            {characters.map((char, i) => (
              <div key={i} className="character-row">
                ...
                <select
                  ...
                  value={(voiceConfig[char.name.trim().toUpperCase()]?.voice_id as string) || ''}
                  onChange={(e) => {
