...
import type { WorldListItem, WorldResponse, VoiceListItem, ApiKeysStatus } from '../types';
...
export default function NewStory() {
  ...
  useEffect(() => {
    ...
    apiFetch('/worlds/').then((data: WorldListItem[]) => {
      setWorlds(data);
  ...
  const loadVoices = () => {
    ...
    apiFetch('/stories/voices')
      .then(setVoices)
      .catch((err: unknown) => setError(`Failed to load voices: ${(err as Error).message}`))
